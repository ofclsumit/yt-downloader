...
export default function YouTubeUrlInput({
  ...
  const handleInputPaste = (e) => {
    ...
    if (result.valid) {
      setUrl(trimmed);
      setInputError(null);
      if (toast) toast.info('YouTube link detected — redirecting to custom session...');
      onLoadYouTube(result.videoId, trimmed);
    }
  };
    ...
    try {
      if (navigator.clipboard && navigator.clipboard.readText) {
        ...
        if (text) {
          ...
          setUrl(trimmed);
          setInputError(null);
          if (toast) toast.info('Link pasted from clipboard');
          const result = extractYouTubeVideoId(trimmed);
          if (result.valid) {
            onLoadYouTube(result.videoId, trimmed);
          }
  ...
  const handleSampleClick = (sample) => {
    ...
    if (sample.type === 'sample') {
      ...
      setUrl(sample.url);
      const result = extractYouTubeVideoId(sample.url);
      if (result.valid) {
        onLoadYouTube(result.videoId, sample.url);
      }
    }
  };
